...
const Charge = {
    getChargesById: (id, cb) => {
        const queryString = 'SELECT ct.charge_type, c.charge_amount FROM charges AS c INNER JOIN charge_types AS ct ON c.charge_type_id=ct.charge_type_id WHERE charge_id=? LIMIT 1;';
    ...
    getChargesByResRoomId: (id, cb) => {
        const queryString = 'SELECT ct.charge_type, c.charge_amount FROM charges AS c INNER JOIN charge_types AS ct ON c.charge_type_id=ct.charge_type_id WHERE res_room_id=?;';
        const queryParams = [id];
        connection.execute(queryString, queryParams, (err, result) => {
            if (err) throw err;
            cb(result);
